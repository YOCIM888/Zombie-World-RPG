...
export function pickRandomLoot(map) {
  ...
  switch (type) {
    ...
    case "medicine": {
      ...
      const medicine = weightedRandom(MEDICINES.filter(m => m.rarity !== "myth"), m => rarityWeights[m.rarity] || 1);
      return { ...medicine };
    }
    case "backpack": {
      const rarityWeights = GAME_CONSTANTS.LOOT.RARITY_WEIGHTS;
    ...
    case "melee": {
      const rarityWeights = GAME_CONSTANTS.LOOT.RARITY_WEIGHTS;
      const melee = weightedRandom(MELEE_WEAPONS.filter(w => w.id !== DEFAULT_ITEM_IDS.melee && w.rarity !== "myth"), w => rarityWeights[w.rarity] || 1);
      return { ...melee, currentDurability: melee.durability };
    }
    case "ranged": {
      const rarityWeights = GAME_CONSTANTS.LOOT.RARITY_WEIGHTS;
      const weapon = weightedRandom(RANGED_WEAPONS.filter(w => w.rarity !== "myth"), w => rarityWeights[w.rarity] || 1);
      return { ...weapon };
    }
    case "ammo": {
    ...
    case "building": {
      const lootableMaterials = BUILDING_MATERIALS;
...
export function generateNpcLoot() {
  ...
  const pools = [
    ...
    () => ({ ...randomItem(MEDICINES.filter(m => m.rarity !== "myth")) }),
  ];
