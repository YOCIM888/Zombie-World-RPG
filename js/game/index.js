...
import { handleIslandAction, handleIslandBankAction, handleIslandBarAction, handleIslandStreetAction, handleIslandBlackMarketAction, handleIslandIntelAction, handleIslandInvestAction } from '../island/index.js';
...
export function handleAction(input) {
  ...
  switch (phase) {
    ...
    case "npc_recycle_ranged":
      handleRecycleRangedConfirm(input);
      break;
    case "npc_recycle_backpack":
      handleRecycleBackpackConfirm(input);
      break;
    case "npc_repair":
    ...
    case "island_black_market":
      handleIslandBlackMarketAction(input);
      break;
    case "island_intel":
      handleIslandIntelAction(input);
      break;
    case "island_street":
    case "island_street_melee":
    case "island_street_ammo":
    case "island_street_ammo_recycle":
    case "island_street_food":
    case "island_street_medical":
    ...
    case "robber_event": {
      const rs = getState();
      const rOptIdx = input - 1;
      if (rOptIdx >= 0 && rOptIdx < rs.options.length && rs.options[rOptIdx].action === "robber_continue") {
        if (rs.currentMap && rs.currentMap.id === "小型岛屿") {
          refreshIslandMenu();
        } else {
